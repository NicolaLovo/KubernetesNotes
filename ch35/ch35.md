...
`etcdctl --endpoints={ETCD_ENDPOINTS} --cacert={ETCD_TRUSTED_CA} --cert={ETCD_CERT_FILE} --key={ETCD_KEY_FILE} snapshot restore {SNAPSHOT_FILE_TARGET_LOCATION}.db --data-dir={DATADIR}`

For some time the kubectl util won't work, sometimes restarting the apiserver(mv all yaml files from their dir to a temp dir and vice versa) and the kubelet is required.

> [NOTE]
> `etcdutl` is the modern version of `etcdctl`, which is now deprecated

The procedure is different for high availability clusters (many master nodes), since it could have different types of topologies:

1. Stacked ETCD topology -> many master nodes, with a load balancer on front. Each master has its ETCD store
2. External ETCD topology -> the ETCD is provisioned externally from the master nodes
   1. higher availability, but higher costs
